...
const roomLinks = [
  { label: 'Villa', href: '/rooms?category=villa' },
  { label: 'Container', href: '/rooms?category=container' },
  { label: 'Wooden', href: '/rooms?category=wooden' },
  { label: 'Conventional', href: '/rooms?category=conventional' }
];
    ...
    <header className={cn('sticky top-0 z-50 transition-all', scrolled ? 'bg-white/80 backdrop-blur-xl shadow-sm' : 'bg-transparent')}>
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 md:px-6">
        <Link href="/" className="flex items-center gap-3">
          <img src="" alt="Logo" className="h-10 w-10 rounded-full bg-forest-700/10 object-cover" />
          <div>
        ...
        <nav className="hidden items-center gap-8 md:flex">
          ...
          <div className="relative">
            <button onClick={() => setRoomsOpen((value) => !value)} className="flex items-center gap-1 text-sm font-medium text-ink hover:text-forest-700">
              Rooms <ChevronDown className="h-4 w-4" />
            </button>
            {roomsOpen ? (
              <div className="absolute left-0 top-full mt-3 min-w-44 rounded-2xl border border-white/70 bg-white p-2 shadow-soft">
                {roomLinks.map((item) => (
                  <Link key={item.label} href={item.href} className="block rounded-xl px-4 py-2 text-sm text-ink hover:bg-forest-50">
                    {item.label}
                  </Link>
                ))}
              </div>
